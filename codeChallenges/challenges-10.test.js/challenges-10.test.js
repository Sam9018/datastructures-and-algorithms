...
const count = (target, input) => {
  let answer = input.reduce( (acc, curr) => {
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length-1; i++){
    if(data[i].gender === 'female' || data[i].gender === 'male') {
      results += data[i].name + ' and ';
    }
  }
  if(data[data.length-1].gender === 'female' || data[data.length-1].gender === 'male') {
    results += data[data.length-1].name;
  }
  return results;
};
